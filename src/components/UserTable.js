  ...
  return (
    <>
      <div className="bg-white rounded-xl shadow-lg border border-agriculture-200">
        {/* Header */}
        <div className="p-6 border-b border-agriculture-200">
          <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
            <h3 className="text-xl font-semibold text-agriculture-800 flex items-center">
              <UserCheck className="w-5 h-5 mr-2 text-agriculture-600" />
              จัดการผู้ใช้งาน
            </h3>
            <div className="relative w-full md:w-80">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 border border-agriculture-300 rounded-lg focus:ring-2 focus:ring-agriculture-500 focus:border-transparent outline-none"
              />
            </div>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-agriculture-50">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-agriculture-500 uppercase tracking-wider">
                  ID
                </th>
                <th className="px-6 py-3 text-left text-xs font-medium text-agriculture-500 uppercase tracking-wider">
                  ผู้ใช้
            ...
            <tbody className="bg-white divide-y divide-agriculture-200">
              {filteredUsers.map((user) => (
                <tr key={user.id} className="hover:bg-agriculture-50">
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-agriculture-900">
                    {user.id}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <div className="flex-shrink-0 h-8 w-8">
                        <div className="h-8 w-8 rounded-full bg-agriculture-500 flex items-center justify-center">
                          <User className="w-4 h-4 text-white" />
                        </div>
                      </div>
                      <div className="ml-3">
                        <div className="text-sm font-medium text-agriculture-900">
                          {user.username}
                        </div>
                      </div>
                    </div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {user.isAdmin ? (
                      ...
                      <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-agriculture-100 text-agriculture-800">
                        <User className="w-3 h-3 mr-1" />
                        User
                      </span>
                    )}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-agriculture-500">
                    {!user.isAdmin && (
                      <div className="flex space-x-2">
                        {user.isBanned ? (
                          <button
                            onClick={() => handleUnbanClick(user.id)}
                            className="inline-flex items-center px-3 py-1 border border-transparent text-xs leading-4 font-medium rounded-md text-white bg-agriculture-600 hover:bg-agriculture-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-agriculture-500"
                          >
